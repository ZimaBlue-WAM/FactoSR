...
from verl.utils.reward_utils import sanitize_reward_extra_infos
...
def compute_reward(data: DataProto, reward_fn: AbstractRewardManager) -> tuple[torch.Tensor, dict[str, Any]]:
    ...
    # Sanitize NaN/Inf (supports mixed spatial_xy/spatial_z/spatial_t)
    if torch.is_tensor(reward_tensor):
        reward_tensor = torch.nan_to_num(reward_tensor, nan=0.0, posinf=0.0, neginf=0.0)
    sanitize_reward_extra_infos(reward_extra_infos_dict)
