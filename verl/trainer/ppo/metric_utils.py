...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Distinguish padding zeros vs original zeros:
    # - Padding zero: token_level_scores.sum(-1)==0 but reward_extra_info["score"]!=0
    # - Original zero: both are 0
    # Corrected: replace padding zeros with reward_extra_info["score"].
    reward_extra_score = None
    if "score" in batch.non_tensor_batch:
        s = batch.non_tensor_batch["score"]
        s = np.asarray(s.cpu().numpy() if isinstance(s, torch.Tensor) else s, dtype=np.float64).ravel()
        n_na = non_aborted_mask.sum().item()
        idx = non_aborted_mask.cpu().numpy()
        if s.size == len(batch):
            reward_extra_score = np.nan_to_num(np.asarray(s[idx], dtype=np.float64), nan=0.0, posinf=0.0, neginf=0.0)
        elif s.size == n_na:
            reward_extra_score = np.nan_to_num(np.asarray(s, dtype=np.float64), nan=0.0, posinf=0.0, neginf=0.0)
    if reward_extra_score is not None and reward_extra_score.size == non_aborted_mask.sum().item():
        seq_np = non_aborted_sequence_score.cpu().numpy().astype(np.float64)
        rew_np = non_aborted_sequence_reward.cpu().numpy().astype(np.float64)
        pad_mask = (np.abs(seq_np) < 1e-9) & (np.abs(reward_extra_score) >= 1e-9)
        corrected_np = np.where(pad_mask, reward_extra_score, seq_np)
        corrected_reward_np = np.where(pad_mask, reward_extra_score, rew_np)
        padding_zero_ratio = float(pad_mask.sum()) / max(1, len(seq_np))
        score_mean_corrected = float(np.mean(corrected_np))
        score_max_corrected = float(np.max(corrected_np))
        score_min_corrected = float(np.min(corrected_np))
        reward_mean_corrected = float(np.mean(corrected_reward_np))
        reward_max_corrected = float(np.max(corrected_reward_np))
        reward_min_corrected = float(np.min(corrected_reward_np))
        pad_mask_full = np.zeros(len(batch), dtype=bool)
        pad_mask_full[non_aborted_mask.cpu().numpy()] = pad_mask
    else:
        padding_zero_ratio = 0.0
        score_mean_corrected = torch.mean(non_aborted_sequence_score).detach().item()
        score_max_corrected = torch.max(non_aborted_sequence_score).detach().item()
        score_min_corrected = torch.min(non_aborted_sequence_score).detach().item()
        reward_mean_corrected = torch.mean(non_aborted_sequence_reward).detach().item()
        reward_max_corrected = torch.max(non_aborted_sequence_reward).detach().item()
        reward_min_corrected = torch.min(non_aborted_sequence_reward).detach().item()
        pad_mask_full = None
    ...
    if pad_mask_full is not None and pad_mask_full.any():
        valid_token_mask = response_mask & (~torch.from_numpy(pad_mask_full).to(response_mask.device).unsqueeze(-1))
    else:
        valid_token_mask = response_mask
    valid_adv = torch.masked_select(advantages, valid_token_mask)
    valid_returns = torch.masked_select(returns, valid_token_mask)

    if use_critic:
        values = batch.batch["values"]
        valid_values = torch.masked_select(values, valid_token_mask)
    ...
    # reward/score_from_tensor: raw from token_level_scores (for debugging)
    # critic/*: exclude padding zeros when reward_extra_info["score"] available
    metrics = {
        "critic/score/mean": score_mean_corrected,
        "critic/score/max": score_max_corrected,
        "critic/score/min": score_min_corrected,
        "reward/score_from_tensor/mean": score_mean,
        "critic/rewards/mean": reward_mean_corrected,
        "critic/rewards/max": reward_max_corrected,
        "critic/rewards/min": reward_min_corrected,
        # adv
    ...
    # Add sub-reward metrics from reward_extra_info
    # For task-specific rewards (r_temporal_consistency, r_depth_consistency, r_proj, etc.),
    # only average over samples where the metric applies (avoids deflating mean with 0s from other data types)
    excluded_keys = {
        "pred_label", "gt_label", "pred", "gt", "answer", "solution",
        "reward_list", "weights", "reward_scores", "reward_details",
        "__num_turns__", "tool_call_counts", "data_source"
    }
    _APPLICABLE_PATTERNS = {
        "r_temporal_consistency": ("spatial_t", "temporal", "camera_motion", "cameramotion", "route_plan", "route", "trajectory", "ego_motion"),
        "r_dynamic_camera": ("spatial_t", "temporal", "camera_motion", "cameramotion"),
        "r_depth_consistency": ("spatial_z", "spatial_3d", "3d", "scannet", "3rscan", "multi_view_3d"),
        "r_proj": ("spatial_z", "spatial_3d", "3d", "scannet", "3rscan", "multi_view_3d"),
        "r_3d_iou": ("spatial_z", "spatial_3d", "3d", "scannet", "3rscan", "multi_view_3d"),
        "r_3d_iou_f1": ("spatial_z", "spatial_3d", "3d", "scannet", "3rscan", "multi_view_3d"),
        "r_reprojection": ("spatial_xy", "correspondence", "point_correspondence", "multi_view_correspondence"),
        "r_contrastive": ("spatial_xy", "correspondence", "point_correspondence", "multi_view_correspondence"),
    }
    data_source_arr = None
    if "data_source" in batch.non_tensor_batch:
        ds = batch.non_tensor_batch["data_source"]
        if isinstance(ds, torch.Tensor):
            data_source_arr = np.array([str(x) for x in ds.cpu().numpy().ravel()])
        else:
            data_source_arr = np.array([str(x) for x in np.asarray(ds).ravel()])
        if len(data_source_arr) != len(batch):
            data_source_arr = None
    ...
    for key in batch.non_tensor_batch.keys():
        ...
        try:
            sub_reward_values = batch.non_tensor_batch[key]
            if isinstance(sub_reward_values, torch.Tensor):
                sub_reward_values = sub_reward_values.cpu().numpy()
            elif not isinstance(sub_reward_values, np.ndarray):
                sub_reward_values = np.array(sub_reward_values)

            if sub_reward_values.dtype.kind in ['f', 'i', 'u']:
                non_aborted_idx = non_aborted_mask.cpu().numpy()
                non_aborted_sub_reward = sub_reward_values[non_aborted_idx]

                if data_source_arr is not None and key in _APPLICABLE_PATTERNS:
                    patterns = _APPLICABLE_PATTERNS[key]
                    ds_na = data_source_arr[non_aborted_idx]
                    applicable_mask = np.array(
                        [any(p in str(d).lower() for p in patterns) for d in ds_na],
                        dtype=bool,
                    )
                    if applicable_mask.any():
                        non_aborted_sub_reward = non_aborted_sub_reward[applicable_mask]

                if len(non_aborted_sub_reward) > 0:
                    metrics[f"reward/{key}/mean"] = float(np.mean(non_aborted_sub_reward))
                    metrics[f"reward/{key}/max"] = float(np.max(non_aborted_sub_reward))
                    metrics[f"reward/{key}/min"] = float(np.min(non_aborted_sub_reward))
