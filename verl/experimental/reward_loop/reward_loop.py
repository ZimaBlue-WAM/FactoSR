...
class RewardLoopManager:
    ...
    def compute_rm_score(self, data: DataProto) -> DataProto:
        ...
        data_len = len(data)
        assert len(outputs_flat) == data_len, (
            f"[reward_loop] outputs_flat len mismatch: got {len(outputs_flat)} expected {data_len}. "
        ...
        infos = [output.get("reward_extra_info", {}) for output in outputs_flat]
        normalized = normalize_reward_extra_infos(infos)
        for k, v in normalized.items():
            assert len(v) == data_len, f"[reward_loop] key '{k}' len {len(v)} != {data_len}"
        non_tensor_batch = dict(normalized)
        reward_extra_keys = sorted(normalized.keys())
