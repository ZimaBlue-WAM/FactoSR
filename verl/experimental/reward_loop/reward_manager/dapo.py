...
class DAPORewardManager(RewardManagerBase):
    ...
    async def run_single(self, data: DataProto) -> dict:
        ...
        # Extract inverse_answer from batch (for temporal consistency / spatial_t reward)
        # First try non_tensor_batch; if absent, decode from inverse_responses tensor
        inverse_answer = data_item.non_tensor_batch.get("inverse_answer", None)
        if inverse_answer is None and "inverse_responses" in data_item.batch:
            inverse_response_ids = data_item.batch["inverse_responses"]
            inverse_prompt_length = data_item.batch.get(
                "inverse_prompt_lengths", torch.tensor(0)
            ).item()
            if "inverse_attention_mask" in data_item.batch:
                inverse_attn_mask = data_item.batch["inverse_attention_mask"]
                valid_inv_len = inverse_attn_mask[inverse_prompt_length:].sum().item()
                valid_inv_ids = inverse_response_ids[:valid_inv_len]
            else:
                valid_inv_ids = inverse_response_ids
            inverse_answer = await self.loop.run_in_executor(
                None, lambda: self.tokenizer.decode(valid_inv_ids, skip_special_tokens=True)
            )
