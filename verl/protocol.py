...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        # Unify batch keys: some workers may have rm_scores/rollout_log_probs/etc., others may not
        # (e.g. spatial_t samples skip async reward). Add placeholder tensors for missing keys before concat.
        if batch_lst[0] is not None:
            all_batch_keys = set()
            for b in batch_lst:
                all_batch_keys.update(b.keys())
            for i, b in enumerate(batch_lst):
                for key in all_batch_keys:
                    if key not in b.keys():
                        ref_batch = next((b2 for b2 in batch_lst if key in b2.keys()), None)
                        if ref_batch is not None:
                            ref_tensor = ref_batch[key]
                            dev = getattr(b, "device", None) or ref_tensor.device
                            placeholder = torch.zeros(
                                (len(b),) + ref_tensor.shape[1:],
        ...
        # Collect all keys from all workers' non_tensor_batch
        all_keys = set()
        for d in data:
            all_keys.update(d.non_tensor_batch.keys())
        
        # For each key, concatenate arrays from all workers, filling missing keys with NaN/None
        non_tensor_batch = {}
        for key in all_keys:
            arrays = []
            for d in data:
                if key in d.non_tensor_batch:
                    ...
                    # Create a fill array for missing key
                    worker_batch_size = len(d)
                    # Find a reference array from a worker that has this key to infer dtype and shape
                    reference_array = None
                    for d2 in data:
                        if key in d2.non_tensor_batch:
                            reference_array = d2.non_tensor_batch[key]
                            break
                    
                    if reference_array is not None:
                        # Infer fill value and dtype from reference array
                        if reference_array.dtype == np.float32 or reference_array.dtype == np.float64:
                            fill_value = np.nan
                            fill_dtype = reference_array.dtype
                        else:
                            fill_value = None
                            fill_dtype = object
                        
                        # Create fill array with same shape (except batch dimension)
                        if len(reference_array.shape) == 1:
                            fill_shape = (worker_batch_size,)
                        else:
                            fill_shape = (worker_batch_size,) + reference_array.shape[1:]
                        
                        fill_array = np.full(fill_shape, fill_value, dtype=fill_dtype)
                    else:
                        # Fallback: use float32 with NaN if no reference found
                        fill_array = np.full(worker_batch_size, np.nan, dtype=np.float32)
                    
                    arrays.append(fill_array)

            # Normalize ndim: if arrays have mixed 1D vs 2D+, flatten higher-dim to 1D (each row -> one object)
            # to avoid "all the input arrays must have same number of dimensions" in concatenate
            ndims = [a.ndim for a in arrays]
            if len(set(ndims)) > 1:
                target_ndim = min(ndims)
                normalized = []
                for a in arrays:
                    if a.ndim > target_ndim:
                        # Flatten: each slice along first dim becomes one object
                        normalized.append(np.array([a[i] for i in range(a.shape[0])], dtype=object))
                    else:
                        normalized.append(a)
                arrays = normalized
                ...
                for k, v in d.meta_info.items():
                    if k == "metrics":
                        ...
                        if k in merged_meta_info:
                            # Special handling for reward_extra_keys: merge lists (union)
                            # Different batches may have different reward combinations
                            if k == "reward_extra_keys" and isinstance(merged_meta_info[k], list) and isinstance(v, list):
                                # Merge lists and remove duplicates, then sort for consistency
                                merged_list = sorted(list(set(merged_meta_info[k] + v)))
                                merged_meta_info[k] = merged_list
                            else:
                                # Ensure consistency for overlapping non-metric keys
                                # assert merged_meta_info[k] == v, f"Conflicting values for meta_info key '{k}'"
                                if merged_meta_info[k] != v and isinstance(merged_meta_info[k], list) and isinstance(v, list):
                                    merged_meta_info[k] = sorted(list(set(merged_meta_info[k] + v)))
                        else:
